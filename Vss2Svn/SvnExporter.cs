...
namespace Hpdi.Vss2Svn
{
    class SvnExporter : Worker
    {
        ...
        private readonly StreamCopier streamCopier = new StreamCopier(128 * 1024);
        ...
        public SvnExporter(WorkQueue workQueue, Logger logger,
        ...
        public void ExportToSvn(string workingCopyPath, bool inheritProjectDir, bool useSvnStandardDirStructure)
        {
            workQueue.AddLast(delegate (object work)
            {
                ...
                LogStatus(work, "Initializing SVN repository");

                // check repository
                var trunkPath = Path.Combine(workingCopyPath, "trunk");
                if (!RetryCancel(delegate {
                    if (!Directory.Exists(workingCopyPath) || (useSvnStandardDirStructure && !Directory.Exists(trunkPath)))
                        throw new DirectoryNotFoundException("SVN working copy is missing");

                    if (!SvnWrapper.ValidateWorkingCopy(workingCopyPath))
                        throw new ArgumentException("SVN working copy is invalid");
                ...
                var svn = new SvnWrapper(workingCopyPath, useSvnStandardDirStructure, logger, parentWindow);
                svn.CommitEncoding = commitEncoding;

                /*
                while (!svn.FindExecutable())
                {
                    var button = MessageBox.Show("Svn not found in PATH. " +
                ...
                if (!RetryCancel(delegate { svn.Init(); }))
                {
                    return;
                }
                ...
                if (commitEncoding.WebName != "utf-8")
                {
                    ...
                    {
                        svn.SetConfig("i18n.commitencoding", commitEncoding.WebName);
                    });
                }
                */
                ...
                foreach (var rootProject in revisionAnalyzer.RootProjects)
                {
                    ...
                    var rootPath = inheritProjectDir ? VssPathMapper.GetWorkingPath(useSvnStandardDirStructure?trunkPath:workingCopyPath, rootProject.Path) : useSvnStandardDirStructure ? trunkPath : workingCopyPath;
                ...
                foreach (var changeset in changesets)
                {
                    ...
                    try
                    {
                        needCommit = ReplayChangeset(pathMapper, changeset, svn, labels);
                    }
                    finally
                    {
                    ...
                    if (needCommit)
                    {
                        LogStatus(work, string.Format("Committing {0} on behalf of {1} with comment \"{2}\"", changesetDesc, GetSvnUserFromVssUser(changeset.User), changeset.Comment));
                        if (CommitChangeset(svn, changeset))
                        {
                            ++commitCount;
                        }
                    }
                    ...
                    if (labels.Count > 0)
                    {
                        foreach (Revision label in labels)
                        {
                            ...
                            {
                                var tagName = GetSvnTagFromVssLabel(labelName);
                                ...
                                if (string.IsNullOrEmpty(tagComment))
                                {
                                    ...
                                    tagComment = forceAnnotatedTags?labelName:DefaultComment;
                                }
                                
                                // prepare tag path
                                var taggedPath = label.Item.IsProject?pathMapper.GetProjectPath(label.Item.PhysicalName): null;
                                ...
                                if (AbortRetryIgnore(
                                    delegate
                                    {
                                        svn.VssLabel(tagName, label.User, GetSvnUserFromVssUser(label.User), tagComment, label.DateTime, taggedPath);
                                    }))
                                {
                                    ++tagCount;
                                }
            ...
            if (revision.Item.IsProject)
            {
                                    ...
                                    if (Directory.Exists(targetPath))
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            */
                                            ...
                                            // svn cares about empty directories
                                            svn.Remove(targetPath, true);
                                            needCommit = true;
                                        }
                                    }
                    ...
                    case VssActionType.Rename:
                        {
                            ...
                            if (targetPath != null && !itemInfo.Destroyed)
                            {
                                ...
                                if (target.IsProject ? Directory.Exists(sourcePath) : File.Exists(sourcePath))
                                {
                                    ...
                                    {
                                        //CaseSensitiveRename(sourcePath, targetPath, svn.Move);
                                    ...
                                    else
                                    {
                                        ...
                                        */
                                        ...
                                        // svn cares about directories, even if empty
                                        //CaseSensitiveRename(sourcePath, targetPath, svn.Move);
                                        svn.Move(sourcePath, targetPath);
                                        needCommit = true;
                                    }
                ...
                if (targetPath != null)
                {
                    if (isAddAction)
                    {
                        ...
                        {
                            logger.WriteLine("{0}: Creating directory {1}",
                                projectDesc, target.LogicalName);
                    ...
                    if (writeProject && pathMapper.IsProjectRooted(target.PhysicalName))
                    {
                        ...
                        foreach (var fileInfo in pathMapper.GetAllFiles(target.PhysicalName))
                        {
                            if (WriteRevision(pathMapper, actionType, fileInfo.PhysicalName,
                                fileInfo.Version, target.PhysicalName, svn))
                            {
        ...
        private string GetSvnUserFromVssUser(string user)
        {
        ...
        private string GetSvnTagFromVssLabel(string label)
        {
            /*
            ...
            */
            var tag = Regex.Replace(label, "[^A-Za-z0-9_-]+", "_");
            return tag;
        }

        private bool WriteRevision(VssPathMapper pathMapper, VssActionType actionType,
            string physicalName, int version, string underProject, SvnWrapper svn)
        {
            var needCommit = false;
        ...
        private void WriteStream(Stream inputStream, string path)
        {
            ...
            using (var outputStream = new FileStream(
                path, FileMode.Create, FileAccess.Write, FileShare.None, 128 * 1024, false))
            {
